...
//       canvas.drawText("稍等片刻", width / 2, height / 2 - textPaint.descent() + textPaint.getTextSize() / 2, textPaint);
    ...
    public void start() {
        ...
        //这里是控制执行时间为2秒
        CountDownTimer countDownTimer = new CountDownTimer(2000, 36) {
            @Override
            public void onTick(long millisUntilFinished) {
                progress = ((2000 - millisUntilFinished) / 2000f) * 360; //总结,圆环度数是360度,所以必须乘以360
            ...
            public void onFinish() {
                progress = 360; //这句必须要有 而且不能改变值 因为弧度为360度
                invalidate();
                if (listener != null) {
